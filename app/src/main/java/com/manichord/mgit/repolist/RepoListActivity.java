...
public class RepoListActivity extends SheimiFragmentActivity {
  ...
  public enum ClickActions { CLONE, CANCEL }
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    RepoListViewModel viewModel =
        ViewModelProviders.of(this).get(RepoListViewModel.class);
    CloneViewModel cloneViewModel =
        ViewModelProviders.of(this).get(CloneViewModel.class);
    ...
    if (uri != null) {
      ...
      try {
        mRemoteRepoUrl = new URL(uri.getScheme(), uri.getHost(), uri.getPort(),
                                 uri.getPath());
      ...
      if (mRemoteRepoUrl != null) {
        ...
        if (!remoteUrl.toLowerCase().endsWith(
                getString(R.string.git_extension))) {
        ...
        List<Repo> repositoriesWithSameRemote =
            Repo.getRepoList(mContext, RepoDbManager.searchRepo(remoteUrl));

        // if so, just open it
        if (repositoriesWithSameRemote.size() > 0) {
          Toast
              .makeText(mContext, R.string.repository_already_present,
                        Toast.LENGTH_SHORT)
              .show();
          Intent intent = new Intent(mContext, RepoDetailActivity.class);
          intent.putExtra(Repo.TAG, repositoriesWithSameRemote.get(0));
          startActivity(intent);
        } else if (Repo.getDir(((SGitApplication)getApplicationContext())
                                   .getPrefenceHelper(),
                               repoName)
                       .exists()) {
          // Repository with name end already exists, see
          // https://github.com/maks/MGit/issues/289
  ...
  protected void onActivityResult(final int requestCode, final int resultCode,
    ...
    case REQUEST_IMPORT_REPO:
      final String path =
          data.getExtras().getString(ExploreFileActivity.RESULT_PATH);
      ...
      builder.setPositiveButton(
          R.string.label_import, new DialogInterface.OnClickListener() {
            @Override
  ...
  private void showCloneView() { binding.getCloneViewModel().show(true); }
