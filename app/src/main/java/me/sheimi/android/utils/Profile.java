...
public class Profile {
  ...
  getProfileSharedPreference(final Context context) {
  ...
  public static String getEmail(final Context context) {
    String userEmailPrefKey =
        context.getString(R.string.pref_key_git_user_email);
    return getProfileSharedPreference(context).getString(userEmailPrefKey, "");
  }

  public static boolean hasLastCloneFailed() { return sHasLastCloneFail; }

  public static Repo getLastCloneTryRepo() { return sLastFailRepo; }
  ...
  public static void setLastCloneSuccess() { sHasLastCloneFail = false; }

  public static synchronized int getTheme(final Context context) {
    // silly, but Android framework want strings as value array for
    // ListPreference
    return Integer.parseInt(getProfileSharedPreference(context).getString(
        context.getString(R.string.pref_key_use_theme_id), "0"));
  }
  ...
  public static String getCodeMirrorTheme(final Context context) {
    final String[] themes =
        context.getResources().getStringArray(R.array.codemirror_theme_names);
    return themes[getTheme(context)];
  }

  public static int getStyledResource(final Context context,
                                      final int unstyled) {
    TypedArray a = context.getTheme().obtainStyledAttributes(
        getThemeResource(context), new int[] {unstyled});
