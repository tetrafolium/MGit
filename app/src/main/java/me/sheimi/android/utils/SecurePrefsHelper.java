...
/**
 * Securely store sensitive data in prefs, encrypting with a key pair that is
 * stored in the Android KeyStore, thus min of API 18 (4.3) is required.
 *
 ...
 *
 * But in this class, all we do is use the generated RSA cert as the "password"
 * used for AES encryption by the SecurePreferences library, by taking the md5
 * of the RSA keys certificates toString()
 */
...
public class SecurePrefsHelper {
  ...
  private static final String KEY_ALGORITHM_RSA =
      "RSA"; // KeyProperties.KEY_ALGORITHM_RSA is only available in API 23, so
  ...
  public SecurePrefsHelper(Context context) throws SecurePrefsException {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2) {
      throw new SecurePrefsException("Min Android version require:" +
    ...
    try {
      ...
      String prefsPassword =
          BasicFunctions.md5(keypair.getCertificate().toString());
      Timber.w("pref password %s", prefsPassword);
      mSecurePrefs =
          new SecurePreferences(context, prefsPassword, SEC_PREFS_FILE_NAME);
  ...
  void generateKeyPair(Context context)
   ...
   */
  public String get(String pref) { return mSecurePrefs.getString(pref, null); }
