...
public class AvatarDownloader extends BaseImageDownloader {
  ...
  private static final String IMAGE_REQUEST_HASH =
      "http://www.gravatar.com/avatar/%s?s=40&d=identicon";
  ...
  public AvatarDownloader(final Context context, final int connectTimeout,
   ...
   * @return true if the use of Gravatar to retrieve Avatar images is enabled,
   *     false otherwise
   */
  protected boolean isGravatarEnabled() {
    ...
    return sharedPreference.getBoolean(
        context.getString(R.string.pref_key_use_gravatar), true);
  }

  /**
   * Retrieves {@link InputStream} of image by URI that uses the avatar://
   * scheme.<br /> Throws {@link UnsupportedOperationException} if the image URI
   * has an unsupported scheme.
   *
   * @param imageUri Image URI
   * @param extra    Auxiliary object which was passed to {@link
   *     DisplayImageOptions.Builder#extraForDownloader(Object)
   *                 DisplayImageOptions.extraForDownloader(Object)}; can be
   * null
   ...
   */
  @Override
  protected InputStream getStreamFromOtherSource(final String imageUri,
                                                 final Object extra)
      throws IOException {
    if (imageUri.toLowerCase(Locale.US).startsWith(AVATAR_SCHEME)) {
      ...
      String gravatarUri =
          String.format(Locale.getDefault(), IMAGE_REQUEST_HASH, hash);
