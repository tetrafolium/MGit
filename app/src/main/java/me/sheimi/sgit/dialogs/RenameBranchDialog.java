...
public class RenameBranchDialog extends DialogFragment
  ...
  public void onClick(View view) {
    ...
    if (newBranchname.equals("")) {
      Toast
          .makeText(mActivity,
                    getString(R.string.alert_new_branchname_required),
                    Toast.LENGTH_LONG)
          .show();
      mNewBranchname.setError(
          getString(R.string.alert_new_branchname_required));
      return;
    }
      ...
      case Repo.COMMIT_TYPE_TAG:
        ...
        for (int i = 0; i < refs.size(); ++i) {
          if (refs.get(i).getName().equals(mFromCommit)) {
            tag = new RevWalk(mRepo.getGit().getRepository())
                      .lookupTag(refs.get(i).getObjectId());
            break;
          }
        }
    ...
    if (fail) {
      mActivity.runOnUiThread(new Runnable() {
        ...
        public void run() {
          Toast
              .makeText(mActivity, "can't rename " + mFromCommit,
                        Toast.LENGTH_LONG)
              .show();
        }
      });
