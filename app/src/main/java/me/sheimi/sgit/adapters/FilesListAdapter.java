...
public class FilesListAdapter extends ArrayAdapter<File> {
  ...
  public View getView(final int position, final View convertView,
    ...
    if (convertView == null) {
      convertView = inflater.inflate(R.layout.listitem_files, parent, false);
    ...
    if (item.isDirectory()) {
      holder.fileIcon.setImageResource(
          Profile.getStyledResource(getContext(), R.attr.ic_folder_fl));
    } else {
      holder.fileIcon.setImageResource(
          Profile.getStyledResource(getContext(), R.attr.ic_file_fl));
    }
    // set if selected
