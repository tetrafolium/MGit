...
public class RepoOperationDelegate {
  ...
  public RepoOperationDelegate(final Repo repo,
  ...
  public void executeAction(final int key) {
  ...
  public void checkoutCommit(final String commitName) {
    CheckoutTask checkoutTask =
        new CheckoutTask(mRepo, commitName, null, new AsyncTaskPostCallback() {
          @Override
  ...
  public void checkoutCommit(final String commitName, final String branch) {
    CheckoutTask checkoutTask = new CheckoutTask(
        mRepo, commitName, branch, new AsyncTaskPostCallback() {
          @Override
          public void onPostExecute(final Boolean isSuccess) {
  ...
  public void mergeBranch(final Ref commit, final String ffModeStr,
    ...
    MergeTask mergeTask = new MergeTask(
        mRepo, commit, ffModeStr, autoCommit, new AsyncTaskPostCallback() {
          @Override
          public void onPostExecute(final Boolean isSuccess) {
            mActivity.reset();
          }
  ...
  public void addToStage(final String filepath) {
  ...
  public void checkoutFile(final String filepath) {
    String relative = getRelativePath(filepath);
  ...
  public void
  deleteFileFromRepo(final String filepath,
                     final DeleteOperationType deleteOperationType) {
    String relative = getRelativePath(filepath);
    DeleteFileFromRepoTask task = new DeleteFileFromRepoTask(
        mRepo, relative, deleteOperationType, new AsyncTaskPostCallback() {
          @Override
          public void onPostExecute(final Boolean isSuccess) {
  ...
  private String getRelativePath(final String filepath) {
