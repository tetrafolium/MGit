...
public class CommitDiffActivity extends SheimiFragmentActivity {
  ...
  public boolean onCreateOptionsMenu(final Menu menu) {
    ...
    shareActionProvider.setOnShareTargetSelectedListener(
        new ShareActionProvider.OnShareTargetSelectedListener() {
          public boolean onShareTargetSelected(final ShareActionProvider source,
                                               final Intent intent) {
            try {
      ...
      Intent intent =
          new Intent(Intent.ACTION_CREATE_DOCUMENT)
              ...
              .putExtra(Intent.EXTRA_TITLE,
                        Repo.getCommitDisplayName(mNewCommit) + ".diff");
  ...
  private class CodeLoader {
    ...
    public void getDiffEntries() {
      ...
      CommitDiffTask diffTask = new CommitDiffTask(
          mRepo, oldCommit, mNewCommit, new CommitDiffResult() {
            @Override
            public void pushResult(final List<DiffEntry> diffEntries,
                                   final List<String> diffStrs,
                                   final RevCommit commit) {
              mDiffEntries = diffEntries;
              mDiffStrs = diffStrs;
              mCommit = commit;
              mLoading.setVisibility(View.GONE);
              mDiffContent.loadUrl(
                  CodeGuesser.wrapUrlScript("notifyEntriesReady();"));
            }
          }, mShowDescription);
